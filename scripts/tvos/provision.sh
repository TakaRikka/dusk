...
# -DCMAKE_XCODE_GENERATE_SCHEME=ON is required: xcodebuild ignores -destination unless a *scheme*
# is passed ("Ignoring provided run destination because no scheme was passed"), and without a
# destination -allowProvisioningDeviceRegistration has no device to register.
cmake -S "$seed_src" -B "$seed_build" -G Xcode -DCMAKE_SYSTEM_NAME=tvOS -DCMAKE_OSX_SYSROOT=appletvos \
      -DCMAKE_OSX_DEPLOYMENT_TARGET=14.5 -DCMAKE_OSX_ARCHITECTURES=arm64 -DCMAKE_XCODE_GENERATE_SCHEME=ON \
...
xcodebuild -project "$seed_build/ProvisionSeed.xcodeproj" -scheme ProvisionSeed -configuration Release \
