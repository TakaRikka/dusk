...
maObj_Wrock_c::~maObj_Wrock_c() {
    // Called every time the actor is unloaded (or deleted)
...
int maObj_Wrock_c::CreateHeap() {
    ...
    // Get the dzb collision date from the archive and initialize it
...
int maObj_Wrock_c::Execute() {
    // Check that the position of the actor is sitting above collision
    ...
    // Move the collider to match the model's transform
    if (mpCollider != NULL) {
...
const ActorProfileDesc maObj_Wrock_c::sProfile = {.name = MAOBJ_WROCK_NAME,
    ...
    .is_delete_function = maObj_Wrock_Execute,
