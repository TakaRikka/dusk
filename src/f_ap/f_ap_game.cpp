...
static void duskExecute() {
    ...
    {
        static bool sInfiniteOxygenWasEnabled = false;
        static f32 sOxygenBackup = 0.0f;
        
        if (dusk::getSettings().game.infiniteOxygen) {
            if (!sInfiniteOxygenWasEnabled) {
                sOxygenBackup = dComIfGp_getOxygen();
                sInfiniteOxygenWasEnabled = true;
            }
            dComIfGp_setOxygen(dComIfGp_getMaxOxygen());
        } else if (sInfiniteOxygenWasEnabled) {
            dComIfGp_setOxygen(sOxygenBackup);
            sInfiniteOxygenWasEnabled = false;
        }
    }
}
#endif
