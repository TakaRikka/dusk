...
int daTagRiverBack_c::Create() {
    ...
    mEventID = (s16)dComIfGp_getEventManager().getEventIdx(this, l_evName, -1);
    // "River flow reversal tag: evid<%d>tool<%d>"
    OS_REPORT("川流れ戻しタグ：evid<%d>tool<%d>\n", mEventID, mMapToolID);
    return 1;
}
...
int daTagRiverBack_c::execute() {
    if (getSwbit() != 0xFF && fopAcM_isSwitch(this, getSwbit()) != 0) {
        return 1;
    }

    if (getSwbit2() != 0xFF && fopAcM_isSwitch(this, getSwbit2()) == 0) {
        return 1;
    }
...
void daTagRiverBack_c::actionWait() {
    u32 result = dComIfGp_checkPlayerStatus0(0, 0x100000);
    if (result != 0) {
        mCount = 0;
...
int daTagRiverBack_c::demoProc() {
    ...
    case WAIT_e:
        ...
        break;
    case SCENE_CHG_e:
        dStage_changeScene(getNextSceneNo(), 0.0f, 0, fopAcM_GetRoomNo(this),
                           0, -1);
        break;
    default:
        dComIfGp_evmng_cutEnd(mStaffID);
...
static int daTagRiverBack_Delete(daTagRiverBack_c* i_this) {
    fopAcM_RegisterDeleteID(i_this, "RiverBack");
    return i_this->_delete();
}

static int daTagRiverBack_Create(daTagRiverBack_c* i_this) {
    fopAcM_RegisterCreateID(i_this, "RiverBack");
...
actor_process_profile_definition g_profile_Tag_RiverBack = {
    ...
    /* Proc Name    */ fpcNm_Tag_RiverBack_e,
    /* Proc SubMtd  */ &g_fpcLf_Method.base,
    /* Size         */ sizeof(daTagRiverBack_c),
    /* Size Other   */ 0,
    /* Parameters   */ 0,
    /* Leaf SubMtd  */ &g_fopAc_Method.base,
    /* Draw Prio    */ fpcDwPi_Tag_RiverBack_e,
    /* Actor SubMtd */ &l_daTagRiverBack_Method,
