...
function(_mod_add_sdl3_actions_headers target_name)
    if (NOT TARGET SDL3::Headers AND NOT TARGET SDL3::SDL3 AND NOT TARGET SDL3::SDL3-static)
        include("${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../extern/aurora/cmake/AuroraDependencyVersions.cmake")
        set(AURORA_SDL3_PROVIDER "auto" CACHE STRING "How to provide SDL3 for the mod SDK")
        include("${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../extern/aurora/cmake/AuroraSDL3Provider.cmake")
    endif ()

    if (TARGET SDL3::Headers)
        target_link_libraries(${target_name} PRIVATE SDL3::Headers)
        return ()
    endif ()

    set(_sdl_target "")
    if (DEFINED AURORA_SDL3_TARGET AND TARGET "${AURORA_SDL3_TARGET}")
        set(_sdl_target "${AURORA_SDL3_TARGET}")
    elseif (TARGET SDL3::SDL3)
        set(_sdl_target SDL3::SDL3)
    elseif (TARGET SDL3::SDL3-static)
        set(_sdl_target SDL3::SDL3-static)
    endif ()
    if (NOT _sdl_target)
        message(FATAL_ERROR "add_mod: FEATURES actions could not provide SDL3 headers")
    endif ()

    get_target_property(_sdl_incs "${_sdl_target}" INTERFACE_INCLUDE_DIRECTORIES)
    if (NOT _sdl_incs)
        message(FATAL_ERROR "add_mod: FEATURES actions: ${_sdl_target} has no INTERFACE_INCLUDE_DIRECTORIES")
    endif ()
    target_include_directories(${target_name} PRIVATE ${_sdl_incs})
endfunction()
...
function(add_mod target_name)
    ...
    set(_supported_features actions game webgpu)
    set(_features "")
    ...
    if (ARG_SOURCES)
        ...
        foreach (_feature IN LISTS _features)
            ...
            if (_feature STREQUAL "actions")
                _mod_add_sdl3_actions_headers(${target_name})
            endif ()
